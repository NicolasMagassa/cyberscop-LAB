...
- [x] **Sécurisation des dépendances backend (SCA) :** Application de correctifs de sécurité directs sur les packages transitifs via `overrides` dans le backend. Sous-lot A1 (mise à niveau de `undici` vers `6.28.0`) et Sous-lot A2 (mise à niveau de `sharp` vers `0.35.3` pour corriger les failles critiques de `libvips`). Validation robuste par 187 tests unitaires Jest, 24 tests Playwright E2E et validation fonctionnelle de traitement d'images (voir le guide dans [technicalBackend.md](./docs/technicalBackend.md#-sécurisation-des-dépendances-backend-sca)).
