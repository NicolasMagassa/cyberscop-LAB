/**
 * Script de gestion pour la page veille.html (Flux de veille cyber)
...
async function renderVeillePageArticles() {
    const loader = document.getElementById('veille-loader');
    const listContainer = document.getElementById('veille-articles-list');
    ...
    try {
        const response = await fetch(`http://localhost:1337/api/veilles?pagination[page]=${currentPage}&pagination[pageSize]=${pageSize}`);
        if (!response.ok) {
    ...
    if (articles.length === 0) {
        listContainer.innerHTML = `
            <div class="text-center py-10 font-mono text-xs text-gray-500 border border-dashed border-gray-300 dark:border-gray-800 rounded-md">
                [SYSTEM WARNING: AUCUN ARTICLE DE VEILLE TROUVÉ]
            </div>
        `;
    } else {
        listContainer.innerHTML = articles.map(generateVerticalVeilleArticleHTML).join('');
    }
    ...
    if (typeof updatePaginationDOM === 'function') {
        updatePaginationDOM(listContainer, currentPage, totalPages, 'cyber-pink', (newPage) => {
            currentPage = newPage;
            if (typeof listContainer.scrollIntoView === 'function') {
                listContainer.scrollIntoView({ behavior: 'smooth' });
            }
            renderVeillePageArticles();
        });
    }
}

/**
 * Génère le code HTML correspondant à un article de veille sous forme de bloc vertical étendu.
 *
 * @param {Object} article - Les données de l'article (id, date, title, description).
 * @returns {string} Le fragment HTML.
 */
function generateVerticalVeilleArticleHTML(article) {
    const formattedDate = typeof formatLongDate === 'function' ? formatLongDate(article.date) : article.date;
    return `
        <article class="bg-white dark:bg-cyber-panel border border-gray-200 dark:border-gray-800 rounded-md p-6 shadow-sm relative overflow-hidden group hover:border-cyber-pink transition-all duration-300">
            <div class="absolute top-0 left-0 w-1 h-full bg-cyber-pink opacity-80 group-hover:w-2 transition-all"></div>
            
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-2 mb-4">
                <span class="text-xs font-mono font-bold text-cyber-pink flex items-center">
                    <i data-lucide="calendar" class="w-3.5 h-3.5 mr-1.5"></i>
                    ${formattedDate}
                </span>
                <span class="text-[10px] font-mono text-gray-400">ID: SEC-FEED-${String(article.id).padStart(4, '0')}</span>
            </div>

            <h3 class="text-xl sm:text-2xl font-bold font-orbitron text-gray-900 dark:text-gray-100 group-hover:text-cyber-pink transition-colors mb-3 leading-snug">
                ${article.title}
            </h3>
            ...
            <div class="flex items-center justify-between border-t border-gray-100 dark:border-gray-800/50 pt-4">
                ...
                <span class="text-[10px] font-mono text-gray-400 flex items-center">
                    <i data-lucide="shield-check" class="w-3.5 h-3.5 mr-1 text-cyber-green"></i> Source vérifiée
                </span>
            </div>
        </article>
    `;
...
if (typeof module !== 'undefined' && module.exports) {
    module.exports = {
        renderVeillePageArticles,
        generateVerticalVeilleArticleHTML,
