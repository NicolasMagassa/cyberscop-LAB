...
/**
 * Récupère, trie et injecte le code HTML des articles de l'IA
 * dans le conteneur principal '#ia-articles-list'.
 *
...
async function renderIAPageArticles() {
    ...
    try {
        ...
        if (articles.length === 0) {
            articles = typeof mockIAData !== 'undefined' ? mockIAData : [];
        }
    } catch (error) {
    ...
    if (sortedArticles.length === 0) {
        ...
        listContainer.innerHTML = sortedArticles.map(generateVerticalIAArticleHTML).join('');
    }
